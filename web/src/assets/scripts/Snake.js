...
export class Snake extends AcGameObject {
    ...
    constructor(info, gamemap) {
        ...
        if(this.id === 1) this.eye_direction = 2; // 左下的蛇初始朝上，右上的蛇朝下
    ...
    check_tail_increasing() { //检测当前回合蛇的长度是否增加
        if(this.step <= 10) return true; //前10回合一定变成
        if(this.step % 3 === 1) return true;//后面每三步边长【因为前面每一步长度+1，后面每三秒+1】
        return false;
    }
    ...
    next_step() { //将蛇的状态变为走下一步
        ...
        //  if(!this.gamemap.check_valid(this.next_cell)) { // 下一步操作撞墙【非法】，蛇die
        //     this.status = "die";
        //  }
    ...
    update_move() { 
        ...
        if(distance < this.eps) { // 走到目标点 
            this.cells[0] = this.next_cell; // 给蛇的cell添加一个新蛇头
            this.next_cell = null;
            this.status = "idle"; //一格走完
